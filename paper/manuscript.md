...
We present a backwards-compatible extension to the STAC Datacube specification introducing three properties: `size` and `href` for paginated access following OGC API conventions, and a `provider` object that encapsulates algorithmic member generation with machine-discoverable OpenAPI definitions. The provider abstraction applies uniformly to temporal calendars, integer ranges, and coded hierarchies. For hierarchical dimensions, a `hierarchy` property provides tree metadata while the provider type determines the navigation strategy -- adding new strategies requires only new provider types, not schema changes. Five conformance levels (Basic, Invertible, Searchable, Hierarchical, Similarity) allow incremental adoption.
...
2. **Algorithmic generation** via a `provider` object that encapsulates deterministic rules for producing dimension members, with machine-discoverable OpenAPI definitions enabling both client-side computation and server-side API calls.
...
We further describe how the provider pattern, combined with vector similarity search on dimension spaces, opens an architectural pathway from traditional exact-coordinate datacube access to concept-proximity navigation -- positioning STAC collections as approximate multidimensional vector indices.
...
**`provider`** (object, RECOMMENDED for large dimensions): A slim reference of the form `{type, href}` where `href` points to the dimension's collection at an OGC API - Records endpoint. Clients retrieve the paginated members endpoint by following the `rel="items"` link returned on that collection response; additional capabilities (`/inverse`, `/search`, `/children`, `/ancestors`) are advertised with their own `rel` values on the same response. Reusing Records link discipline — rather than introducing a new dimension-specific URL property — keeps the proposal within the existing OGC linking vocabulary and simplifies client implementations. The full provider definition (configuration, output schema, invertibility, hierarchy, language support, conformance URIs) lives on that collection response and is not duplicated inline.

Both properties are backwards-compatible additions. Existing clients that read only the `values` array continue to work for small dimensions. Servers may provide inline `values` for dimensions below an implementation-defined threshold (recommended: 1000 members) while using `provider` for larger dimensions.

For backwards compatibility with STAC clients that cannot follow `links[]`, a dimension-level **`href`** property remains DEFINED as a deprecated transitional alias pointing directly at the paginated items endpoint. When emitted, it MUST resolve to the same URL as `provider.href` + `rel="items"`. New implementations SHOULD omit it; a future major revision of this specification MAY remove it.
...
### 3.2 The Provider Object

Many dimensions follow deterministic rules that make explicit enumeration wasteful. We introduce a `provider` property on any dimension object, applicable to temporal, spatial, and thematic dimensions alike.

The provider object contains the following fields:

- **`type`** (string or URI, REQUIRED): A short identifier for well-known algorithms (`dekadal`, `pentadal-monthly`, `pentadal-annual`, `iso-week`, `integer-range`, `grid-index`) or a full URI for custom providers. Well-known types resolve to registered OGC Definition URIs.

- **`api`** (string, URI, CONDITIONAL): An OpenAPI definition URI. Required for custom providers; implicit for well-known types. Machine clients fetch this specification to discover the generation endpoint and its parameters.
...
- **`output`** (JSON Schema, REQUIRED): The type and structure of each produced member, also defined per JSON Schema 2020-12. Supports simple types (`string` with `format: "date-time"`), integers, and structured objects with multiple fields.

- **`invertible`** (boolean, OPTIONAL, default false): Whether the provider exposes `/inverse` — a well-defined, deterministic lookup from any domain value to exactly one dimension member. See Section 3.3 for the mathematical semantics.
...
- **`hierarchical`** (boolean, OPTIONAL, default false): Whether the provider supports Hierarchical conformance level -- `/children`, `/ancestors`, and the `?parent=` filter on `/members`. This property should be `true` when the dimension declares a `hierarchy` property. See Section 3.6.

- **`navigable`** (boolean, OPTIONAL, default false): Whether the provider supports per-member navigation links (`rel:children`, `rel:ancestors`) when clients request them via `?links=true`. Requires `hierarchical: true`. By default, member-level links are suppressed to minimize response size; response-level pagination links (`self`, `next`, `prev`) are always included.
...
```json
{
  "cube:dimensions": {
    "time": {
      ...
      "provider": {
        "type": "dekadal",
        "api": "http://www.opengis.net/def/provider/ogc/0/dekadal/openapi.json",
        "parameters": {},
        "output": {
          "type": "object",
          "properties": {
...
Legacy clients that do not recognize `provider`, `size`, or `href` ignore these properties per standard JSON processing rules. Clients that do recognize `href` can follow it to retrieve paginated members without any knowledge of the generation algorithm. For example, a legacy client requesting `?format=datetime` receives standard ISO dates (`["2024-01-01", "2024-01-11", "2024-01-21", ...]`) -- a valid irregular temporal dimension indistinguishable from a traditional `values` array. Provider-aware clients can additionally use `/inverse`, `/search`, and native format negotiation.

### 3.3 Provider Invertibility and Dimension Integrity

A provider defines a forward function *f* from the value domain *V* (e.g., all calendar dates) to the set of dimension members *M* (e.g., all dekads). This function is *total* — every element of *V* maps to exactly one member — and *surjective* — every member has at least one preimage in *V*. It is not, in general, injective: many dates belong to the same dekad, and many elevation values fall within the same band. The provider therefore induces an equivalence relation on *V*, partitioning the domain into disjoint intervals, each corresponding to one dimension member. This partition is precisely the *quantization function* of information theory and the *partitioning predicate* of distributed storage systems (Hive, Iceberg, Delta Lake).

We refer to providers that support this partition semantics as *invertible* providers, using the conformance level name Invertible (Section 3.5). In the `provider` object, these providers set `invertible: true`. The provider's forward function is a total surjection from domain values to members, and its `/inverse` endpoint computes the unique member *m* such that *v* falls within the interval defined by *m*. Inverse lookup is thus well-defined and O(1) for all providers implemented here.

When a value falls outside the provider's declared extent — for example, an elevation of 9,000 m in a dimension bounded by 0–8,848 m — the provider reports `valid: false` and, where applicable, identifies the nearest valid member to assist data repair workflows. This failure mode is analogous to a foreign key constraint violation in relational databases: the value exists syntactically but cannot be placed within any dimension member.
...
The invertibility property serves three purposes in datacube management. First, it enables *ingestion validation*: when a new item is inserted, its dimension values are submitted to `/inverse`, and the `on_invalid` policy governs the outcome — `reject` enforces strict referential integrity, `accept` allows schema-on-read expansion, and `warn` flags anomalies without blocking ingestion. Second, it enforces *cross-collection consistency*: multiple collections sharing the same `provider.type` and endpoint are guaranteed to use identical member boundaries, eliminating the off-by-one errors that arise when independent systems compute period boundaries from shared but under-specified conventions. Third, it supports *data quality monitoring*: the ratio of valid to invalid inverse results across an ETL pipeline is a direct measure of alignment between incoming data and the declared dimension structure. For invertible providers, the `/inverse` endpoint supports both single-value queries (GET) and batch operations (POST) for efficient pipeline integration.

### 3.4 Search and Similarity

Providers may optionally expose a search capability supporting multiple protocols. Deterministic protocols (`exact`, `range`, `like`) enable traditional filtering on produced or stored dimension members. The `vector` protocol opens an architectural pathway to AI-powered dimension navigation: when dimension members are associated with embedding vectors, clients can search by similarity rather than exact coordinates, effectively navigating the datacube by concept proximity.

This positions STAC collections as approximate multidimensional vector indices. Each dimension with a searchable provider contributes one axis of a multi-dimensional index. A client searching across N dimensions receives items ranked by combined similarity across all axes -- a fundamentally different access pattern from traditional exact-coordinate subsetting.

### 3.5 Adoption Levels and Building Blocks

The specification groups capabilities into four normative **adoption levels** plus one informative architectural hook. The normative levels are realised by five OGC Building Blocks; adopters consume Building Blocks, while readers and reviewers discuss adoption levels. The crosswalk is explicit:

| Level | Capabilities | Realised by Building Block(s) | Conformance class URI stem | Requirement |
|---|---|---|---|---|
| Basic | `/items` + `/extent` + `/conformance` + `/queryables` | `dimension-collection`, `dimension-member`, `dimension-pagination` | `…/conf/dimension-collection`, `…/conf/dimension-member`, `…/conf/dimension-pagination` | MUST support |
| Invertible | + `/inverse` | `dimension-inverse` | `…/conf/dimension-inverse` | Invertible providers only |
| Searchable | + `/search` (exact, range, like) | `dimension-collection` (query capability) | — (covered by dimension-collection conformance) | SHOULD support |
| Hierarchical | + `/children` + `/ancestors` + `?parent=` filter | `dimension-hierarchical` | `…/conf/dimension-hierarchical` | Required when the dimension declares a `hierarchy` property |
| Similarity | + `/search?similar=...` (vector embedding, k-NN) | `dimension-similarity` | `…/conf/dimension-similarity` | Optional; servers without a backing index MUST return the prescribed 501 problem-detail |

Conformance class URI stems resolve under `http://www.opengis.net/spec/ogc-dimensions/1.0/`. Basic is the minimum requirement for any provider implementation. Invertible applies to providers that implement the partition semantics described in Section 3.3 — that is, providers for which every domain value maps deterministically to exactly one member. Temporal calendars (dekadal, pentadal) and integer ranges satisfy this property; the provider field `invertible: true` signals this capability. Its concrete use cases are ingestion validation (rejecting items whose dimension values fall outside any valid member interval), cross-collection consistency (guaranteeing identical member boundaries across collections sharing the same provider type), and data quality monitoring (tracking the valid-to-invalid ratio of incoming values as a pipeline health metric). Searchable is recommended for any non-trivial dimension. Hierarchical is required when the dimension declares a `hierarchy` property and is orthogonal to Invertible — a hierarchical nominal dimension may independently be invertible if its member codes are canonical identifiers (for example, ISO 3166-1 alpha-3 country codes, where the forward function maps an incoming code to its matching member or reports an error). Similarity is the fifth normative conformance level: it exposes `/search?similar=...` (vector embedding k-NN over dimension members) and pins a stable target for future embedding-aware implementations. Servers without a backing vector index MUST return the prescribed `501 Not Implemented` problem-detail (`type: …/errors/similarity-not-implemented`); the conformance class URI is still advertised at `/conformance` so clients can negotiate the capability surface against the spec rather than guess. This pattern keeps the architectural pathway pinned without inflating "Similarity" claims with optional behaviour.
...
The Hierarchical conformance level is orthogonal to all other conformance levels. A provider that declares `hierarchical: true` exposes `/children`, `/ancestors`, and the `?parent=` filter; it may independently be invertible (declaring `invertible: true` to also expose `/inverse`), searchable, or both. The world-admin demo dimension in the reference implementation illustrates this independence: the static tree provider is hierarchical and searchable but not invertible, because continent and country codes are not deterministically derivable from spatial coordinates without additional metadata. A dimension of ISO 3166-1 alpha-3 country codes with a lookup-table provider would be both hierarchical and invertible if the lookup function is surjective.
...
A live deployment is available on the FAO Agro-Informatics Platform review environment, integrated as an extension of the GeoID catalog platform (https://github.com/un-fao/geoid). The deployment demonstrates all provider endpoints with full pagination, inverse mapping, and search capabilities. The Swagger UI is accessible at https://data.review.fao.org/geospatial/v2/api/tools/docs, and the `href` links in the worked examples point directly to these live endpoints. The GeoID platform hosting the extension publishes a broader OGC API surface on the same service stack — Records, Features, Coverages, Styles, Maps, Processes, the OGC Joins draft, and a 3D GeoVolumes route shell — so the dimensions API composes with the rest of the OGC portfolio under a single `/conformance` aggregate rather than standing alone. This multi-extension coexistence is itself a validation point for the building-block approach: each extension declares its own conformance URIs while sharing resolution, authentication, and tenancy layers through the platform's `ExtensionProtocol` / `OGCServiceMixin` seam.

The implementation includes six provider types:

- **DekadalProvider**: 36 periods/year, invertible, searchable (exact, range, like)
- **PentadalMonthlyProvider**: 72 periods/year (month-aligned variant used by CHIRPS/FAO), invertible, searchable
- **PentadalAnnualProvider**: 73 periods/year (year-aligned variant used by GPCP/CPC/NOAA), invertible, searchable
- **IntegerRangeProvider**: configurable step, invertible, searchable (exact, range)
- **StaticTreeProvider**: in-memory recursive tree; Hierarchical and Searchable conformance; not invertible
- **LeveledTreeProvider**: extends `StaticTreeProvider` with a `?level=` filter parameter, supporting condition-based level queries (`?level=0` for root members, `?level=1&parent=X` for children at a specific level)

The temporal and integer-range providers implement the Basic, Invertible, and Searchable conformance levels. The `StaticTreeProvider` implements the Basic, Searchable, and Hierarchical conformance levels. The `LeveledTreeProvider` extends these with level-based filtering, demonstrating the leveled hierarchy strategy: clients may query by level (obtaining all members at a given depth) or by parent (navigating the tree), or combine both.
...
The pentadal providers are validated against CHIRPS (monthly variant) and GPCP (annual variant) period definitions. The integer-range provider is validated against standard binning operations.
...
Inverse computation is O(1) per value for temporal providers (direct arithmetic from date components). Batch inverse processes 1,200 values in 3.2 ms, making ingestion validation of millions of records practical within ETL pipeline latency budgets.
...
- `temporal-dekadal` — `DekadalProvider`, 36 periods/year, extent 1950-01-01 to 2050-12-31 (3,636 members across 100 years)
- `temporal-pentadal-monthly` — `PentadalMonthlyProvider`, 72 periods/year, month-aligned (7,272 members)
- `temporal-pentadal-annual` — `PentadalAnnualProvider`, 73 periods/year, year-aligned (7,373 members)

Each dimension's `provider.type` is a distinct identifier (`dekadal`, `pentadal-monthly`, `pentadal-annual`), enabling clients to detect calendar system mismatches before joining data. The invertible provider's `/inverse` endpoint makes the incompatibility explicit and machine-checkable: `GET /dimensions/temporal-pentadal-monthly/inverse?value=1950-P12` and `GET /dimensions/temporal-pentadal-annual/inverse?value=1950-A12` return different date ranges, demonstrating that the same numeric position maps to different calendar intervals in the two systems.
...
This use case corresponds architecturally to vocabulary services such as AGROVOC [29] (FAO agricultural thesaurus, 40,000+ concepts) and GEMET, which combine hierarchical SKOS concept schemes with full-text search. The provider API bridges these vocabulary systems to the STAC dimension model: any SKOS concept scheme can be exposed as a hierarchical, searchable dimension provider through an adaptor that maps `skos:narrower` to `/children` and `skos:prefLabel` pattern queries to `/search?like=`. Clients need not learn SPARQL or the SKOS data model; they use the same dimension API they already know.

### 5.5 Elevation Bands — Bijective Provider and Ingestion Validation
...
**Solution.** The `elevation-bands` dimension uses `IntegerRangeProvider(step=50)` with extent 0–8848 m. It implements the Invertible conformance level: `/inverse` accepts a raw elevation value and returns the enclosing band, its index, and the band's boundary range.
...
Multiple collections sharing the same temporal cadence — NDVI composites, precipitation estimates, and soil moisture indicators, all at dekadal frequency — must use identical temporal coordinates to be jointly queryable. Without a shared provider definition, independent systems compute period boundaries separately and may introduce off-by-one errors at month boundaries (particularly for the third dekad, which varies between 8 and 11 days depending on the month). By referencing the same `provider.type: "dekadal"` with the same endpoint, all collections in the same platform are guaranteed to use coordinates produced by the same algorithm. The `/inverse` endpoint makes this guarantee enforceable at ingestion time: when a new item is inserted into any collection in the system, its temporal value is submitted to `/inverse`, and a mismatch with the expected dekadal boundary produces a hard rejection rather than a silent data quality issue.
...
## 6. Standardization Pathway and Governance

Because STAC became an OGC Community Standard in October 2025 (OGC 25-004, 25-005), the work presented here sits at the intersection of two governance communities rather than being routed solely through one. We make the governance split explicit.

**Primary normative home: STAC Datacube Extension.** The JSON Schema additions — `size`, the slim `provider` reference in `cube:dimensions`, `hierarchy`, and the two new dimension type values `nominal` and `ordinal` — are submitted as a pull request to the `stac-extensions/datacube` repository. The STAC community extension process is the lighter-weight path with the shorter cycle time, the existing domain precedent (e.g., `eo`, `sar`, `pointcloud`), and the validators and catalog tooling already deployed across the community. A single authoritative schema prevents the fragmentation that would follow from maintaining independent OGC and STAC definitions of the same fields.

**Companion OGC-facing artifact: the five Building Blocks.** The server-side capabilities — `dimension-collection`, `dimension-member`, `dimension-pagination`, `dimension-inverse`, `dimension-hierarchical` — are published as OGC Building Blocks profiling OGC API - Records (OGC 20-004) and OGC API - Common Part 2 (OGC 19-072). Each Building Block declares a conformance class URI under `http://www.opengis.net/spec/ogc-dimensions/1.0/conf/` and is independently adoptable. Change control for the Building Blocks is handled by the OGC GeoDataCube Standards Working Group (SWG charter OGC 22-052), which is the venue best positioned to coordinate this work with the rest of the GDC specification surface. The Building Blocks reference the STAC schema as their normative metadata model, so the two artifacts remain synchronized by construction rather than by duplication.

**Phased adoption steps:**

1. **Community publication.** This paper and the companion repository (specification, reference implementation, worked examples, OGC Building Blocks) serve as the initial community contribution.
2. **STAC Community Extension pull request.** JSON Schema changes land in `stac-extensions/datacube`, referencing the Testbed 19/20 findings and the worked examples in this repository.
3. **OGC GeoDataCube SWG engagement.** The Building Blocks are presented to the GDC SWG as a Change Request Proposal. Because STAC is now an OGC Community Standard and GDC SWG has already adopted `cube:dimensions` as its metadata model, the Building Blocks add capability without re-opening the metadata-schema question.
4. **OGC Naming Authority.** Calendar algorithm definitions (`dekadal`, `pentadal-monthly`, `pentadal-annual`) are registered as OGC Definition URIs with SKOS RDF descriptions, so that providers and clients can refer to canonical algorithm identifiers across the two communities.
5. **OGC Innovation Program.** "Scalable Dimension Members" is proposed as a thread topic for a future OGC Testbed, with the reference implementation as a testbed component — providing the interoperability evidence base needed to graduate the Building Blocks from `under-development` to `stable`.

A natural objection is that extending the STAC Datacube schema is "too opinionated" -- that dimension pagination and generation should live at a higher abstraction level, such as a standalone OGC Building Block or a separate API profile. The governance split above is our answer. The STAC Datacube Extension is the correct home for the *metadata schema* for three reasons. First, the GDC SWG charter explicitly adopts STAC `cube:dimensions` as its metadata model; any solution not expressed in that schema requires consumers to reconcile two independent dimension metadata sources. Second, the properties we add are strictly optional and backwards-compatible: existing clients and validators ignore unknown properties per JSON processing rules, so the extension imposes zero cost on implementations that do not need it. Third, the STAC community extension ecosystem already provides a proven governance model for domain-specific additions, with clear promotion paths from community extension to official extension. The five Building Blocks are the correct home for the *API contracts* (pagination, inverse, hierarchical navigation) because those are API-surface concerns that OGC already governs through Building Blocks and Standards Working Groups. Keeping schema and API governance in their respective natural homes — synchronized by a single reference implementation and a single test corpus — is the opposite of fragmentation: it is the minimal change that respects what each community already does well.

## 7. Discussion

The provider pattern addresses a genuine gap confirmed by OGC's own testbed program. The approach is deliberately minimal: three new properties on an existing schema, each optional and backwards-compatible. Existing clients and servers are unaffected. Adoption can be incremental -- a server might initially support only `size` and `href` (pagination without generation), then add providers for specific dimension types as needed.

The decision to use JSON Schema 2020-12 for both `parameters` and `output` fields avoids introducing a new type system. OpenAPI specifications already use JSON Schema internally, creating a natural alignment between the provider metadata in collection descriptions and the actual API contracts of provider endpoints.
